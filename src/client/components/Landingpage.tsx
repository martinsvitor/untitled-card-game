...
function Landingpage() {
    ...
    useEffect(() => {
        ...
        socket.on('new-player', (updatedGame: GameEngine) => {
            ...
            setGameList([
                ...gameList.filter((game) => game.id !== updatedGame.id),
                updatedGame,
            ]);
        });
    ...
    return (
        <div>
            Landingpage
            <button onClick={() => socket.emit('create-game', 4)}>
                Create game
            </button>
            {gameList.length ? (
